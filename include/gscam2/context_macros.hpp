...
#define CXT_MACRO_LOG_SORTED_PARAMETER(cxt_ref, n, t, \
    d)  ps.emplace_back(std::string(#n).append(" = ") \
...
#define CXT_MACRO_CHECK_CMDLINE_PARAMETERS(node_ref, all_params) { \
  ...
  RCLCPP_INFO(node_ref.get_logger(), "**** ERROR: Undefined command line parameter: %s", \
        npo.first.c_str()); \
  } \
}
